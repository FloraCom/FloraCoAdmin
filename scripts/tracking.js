...
import { updateItemDisplayPrint, getItem, closeCard, openCard, printInvoice } from '../scripts/FloraCoFunctions.js';

window.updateItemDisplayPrint = updateItemDisplayPrint;
...
document.getElementById('searchButton').addEventListener('click', ()=> {
  ...
  if (String(search).trim() !== "" ) {
      loadDataById(String(search).trim().toUpperCase(), ordes => {
      display(ordes);
    });
  }else{
    display(ords);
...
async function loadData(callBack){
  ...
  const q = query(usersCollection, where("status", "==", 1), orderBy("id", "desc"), limit(10));
        ...
        li.innerHTML = `
          <div class="orderItem">
            <div class="card">
                ...
                <p class="itemShowText" onclick='updateItemDisplayPrint(${js},  ${index});'>print</p>
                <p class="itemShowArrow" onclick='updateItemDisplayPrint(${js},  ${index});'><i class="fi fi-ss-print"></i></p>
            </div>
          </div>
          <div class="hr">
            <hr>
